...
def _build_dataset_info(record: dict, dataset_filename: str) -> dict:
    required_keys = {"messages", "chosen", "rejected"}
    if not required_keys.issubset(record):
        raise ValueError("Unsupported DPO dataset format. Expected `messages`, `chosen`, and `rejected`.")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run UserAssist DPO via LLaMA-Factory.")
    ...
    parser.add_argument("--dataset-name", default="userassist_dpo")
    ...
    yaml_path = run_dir / "train_dpo.yaml"
    yaml_text = f"""
### model
